...
def ddivision(to,v1,v2,f=0):
    ...
    txt=bcopyvar(v1,nx)+bcopyvar(v2,ny)+bgo(z)+'[-]'
    txt+=bgo(t)+'[-]+['+bcont(nx,nx,'-',ny)
    txt+=bgo(z)+'+'+bis(t,nx,'<',ny)+bis(t,t,'not',1)
    txt+=bgo(t)+'.]'
    if f:
        txt+=bcopyvar(nx,z)
    else:
