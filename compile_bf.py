...
def findluse(i=-1):
    ...
    while 1:
        if i+p not in luse:
            return i+p
        if (i-p not in luse )and( not i-p<1):
            return i-p
        p+=1
def ofindluse():
    i=0
...
def bdelvar(name):
    if name in varl:
        luse.remove(varl[name])
        del varl[name]
    return ''

def bcopyvar(whh,to,add=0):
    wh=dvarad(whh)
    t=findluse(wh)
    luse.append(t)
...
def ddivision(to,v1,v2,f=0):
    ...
    if f:
        txt+=bcont(nx,[nx],'+',[ny])+bcopyvar(nx,z)
    else:
        txt+=bgo(z)+'-'
...
def bcont(tow,v1,yun,v2):
    txt=''
    
    if yun=='+':
        txt=daddvar(tow,v1,v2)
    elif yun=='-':
        txt=dpassnum(tow,v1,v2)
    elif yun=='opp':
        if v1==tow:
            nv=findluse()
            luse.append(nv)
        ...
        txt=dmultiplication(tow,v1,v2)

    elif yun=='/':
        txt+=ddivision(tow,v1,v2)
            
    elif yun=='%':
        txt+=ddivision(tow,v1,v2,1)
...
def bisbig(to,v2,v1):
    ...
    luse=[item for item in luse if item not in tl]
    luse.remove(t0)
...
def bis(to,v1,yun,v2):
    ...
    if yun=='>':
        ...
        txt+=bisinlist(to,v1,v2)
    return txt   
    
'''
def bisbig(to,v1,v2,equal=0):#v1>v2?
    #print(luse)
    a=findluse()
    luse.append(a)
    b=findluse()
    ...
    print(bfi)
    if equal:
        txt+=bgo(a)+'[-'+bgo(b)+'-'+bgo(a)+']'
    else:
        txt+=bgo(a)+'.[-'+bgo(b)+'-['+bgo(t)+']'+bgo(a)+'].'
...
def checkif(nt):
    ...
    if not isifend:
        return
    if nt[0]=='elif' or nt[0]=='else':
        return
    x=isifend[1][0]
    ix=isifend[0]
    luse.remove(x)
    if isifend[1][2]:
        luse.remove(isifend[1][1])
    del varl[(str(ix)+'if')]
    isifend=[]
    

'----------if-------------'
def dwhilestart(x,ix):
    newx=dvarad(x)
    varl[(str(ix)+'while')]=x
    txt=bgo(newx)+'['
    return txt
...
def dwhileend(ix):
    x=varl[varl[(str(ix)+'while')]]
    txt=bgo(x)+']'
    del varl[(str(ix)+'while')]
    return txt

def dforstart(name,lt,ix):
    vi='fori'+str(ix)
    vwh='forwh'+str(ix)
...
def bprinti(varn):
    ...
    luse=[item for item in luse if item not in ads]
...
def bian(t,xian=0):
    global isifend
    global bfi,varl,luse,ix,btxt,vlistl,funcsave,infunc
